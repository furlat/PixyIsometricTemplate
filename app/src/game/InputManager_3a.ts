...
/**
 * InputManager_3a - Mesh-first input handler for Phase 3A
 * 
...
export class InputManager_3a {
  ...
  public initialize(): void {
    ...
    console.log('InputManager_3a: Event handlers initialized')
  }
  ...
  public updateMovement(deltaTime: number): void {
    ...
    // For Phase 3A, we handle movement on keydown immediately
  ...
  /**
   * Handle mesh-based events (for compatibility with BackgroundGridRenderer_3a)
   */
  public handleMeshEvent(eventType: 'move' | 'down' | 'up', vertexX: number, vertexY: number): void {
    // For Phase 3A, we handle mouse events through the grid renderer
    // This method provides compatibility if needed
    console.log(`InputManager_3a: Mesh event ${eventType} at vertex (${vertexX}, ${vertexY})`)
  ...
  public handleMeshWheelEvent(vertexX: number, vertexY: number, event: any): void {
    // For Phase 3A, we don't implement zoom yet
    // This method provides compatibility for future expansion
    console.log(`InputManager_3a: Wheel event at vertex (${vertexX}, ${vertexY})`, event.deltaY)
  }
  ...
  public getDebugInfo(): any {
    return {
      ...
      navigationOffset: gameStore_3a.navigation.offset,
      isDragging: gameStore_3a.navigation.isDragging,
      meshData: {
        cellSize: gameStore_3a.mesh.cellSize,
        dimensions: gameStore_3a.mesh.dimensions,
        vertexCount: gameStore_3a.mesh.vertexData ? gameStore_3a.mesh.vertexData.length / 2 : 0
      }
    }
  }
  ...
  public destroy(): void {
    ...
    // For Phase 3A, we keep it simple
